...
class Output(Chart):
    ...
    def execute(self):
        week_date = self.get_week_date_df()
    ...
    def sorting_data(self, year, month_week, start_date, end_date):
        ...
        for item in mach_list:
            ...
            sql = f"""
                SELECT
                    wo.Id AS WorkOrder, wo.PartNo, wo.ProductItem, rc.InspectionDate AS WorkDate, rc.MachineName AS Machine, rc.LineName AS Line,
                ...
                GROUP BY wo.Id, wo.PartNo,
                    wo.ProductItem, rc.InspectionDate, rc.MachineName, rc.LineName, rc.Id, rc.period,
            ...
            if not counting_df.empty and not wo_info_df.empty:
                ...
                for _, row in data_df.iterrows():
                    try:
                        ...
                        create_at, plant, branch, belong_to)
                        ...
                        GETDATE(), '{sPlant}', '{sPlant3}', '{belong_to}')
                        """
                        print(insert_sql)
    ...
    def ipqc_data(self, year, month_week, start_date, end_date):
        ...
        sql = f"""
            SELECT ipqc.RunCardId Runcard, ipqc.OptionName, ipqc.InspectionStatus, ipqc.InspectionValue, ipqc.Lower_InspectionValue, ipqc.Upper_InspectionValue
        ...
        for index, row in counting_df.iterrows():
            ...
            for option in OptionName:
                ...
                ipqc[option] = {'value': value, 'status': status, 'lower': lower, 'upper': upper}
            ...
            sql = f"""
                        ...
                        Tensile_Value,Tensile_Limit,Tensile_Status,
                        Elongation_Value,Elongation_Limit,Elongation_Status,
                        Roll_Value,Roll_Limit,Roll_Status,
                        Cuff_Value,Cuff_Limit,Cuff_Status,
                        Palm_Value,Palm_Limit,Palm_Status,
                        Finger_Value,Finger_Limit,Finger_Status,
                        FingerTip_Value,FingerTip_Limit,FingerTip_Status,
                        Weight_Value,Weight_Limit,Weight_Status,
                        Length_Value,Length_Limit,Length_Status,
                        Width_Value,Width_Limit,Width_Status,
                        Pinhole_Value,Pinhole_Limit,Pinhole_Status, create_at)
                        Values({year},'{month_week}','{runcard}',{tensile_value},'{tensile_limit}','{tensile_status}',
                        {elongation_value},'{elongation_limit}','{elongation_status}',
                        {roll_value},'{roll_limit}','{roll_status}',
                        {cuff_value},'{cuff_limit}','{cuff_status}',
                        {palm_value},'{palm_limit}','{palm_status}',
                        ...
                        {length_value},'{length_limit}','{length_status}',
                        {width_value},'{width_limit}','{width_status}',
                        {pinhole_value},'{pinhole_limit}','{pinhole_status}', GETDATE()
                        )
                    """
            print(sql)
            mes_olap_db.execute_sql(sql)
